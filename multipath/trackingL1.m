function [trackResults, channel]= trackingL1(fid, channel, settings, signalType)
               ...
               if (dataAdaptCoeff==1)
                ...
                if (dataAdaptCoeff==4)   
                 ...
                 if (dataAdaptCoeff==8)    
                    [data, samplesRead] = fread(fid, [2, blksize], settings.dataType);
                    rawSignal = data(1,:) + data(2,:).*i; %Inphase and Quadrature
                    % if strcmp(signalType, "real")
                    %     noise = randn(1,length(data));
                    % elseif strcmp(signalType, "complex")
                    %     noise = sqrt(1/2).* (randn(1,length(data)) + 1j* randn(1,length(data)));
                    % end
                    CN0 = 45;
                    fs = settings.samplingFreq;
                    snr = 10^((CN0-10*log10(fs))/10);
                    signal_power = sum(abs(rawSignal).^2)/length(rawSignal);
                    noise_power = signal_power / snr;
                    noise = sqrt(noise_power) * randn(1,length(rawSignal));
