function [posEst] = arsSearch(usrInfo, satInfo, rawSignal,...
        caCodeSample, nIter, dMax, dMin, contraction)
%% Memory Allocation
...
r = generateCAFs(usrInfo, satInfo, rawSignal, caCodeSample);
J_ant = r;
...
for it = 1:nIter-1       
    ...
    usrInfo(1:3) = estPos(it,:) + d*(2*rand(1,3)-1);%[d*(2*rand(1,2)-1), 0];
    usrInfo(4) = estDt(it,:) + d*1e-9*randn;
    r = generateCAFs(usrInfo, satInfo, rawSignal, caCodeSample);
    J = r;
    cost(it+1) = r;
...
%% plot path
% open("result/multipathErrorVSposError/cafs_gird_2d_at_1ms.fig")
% hold on
