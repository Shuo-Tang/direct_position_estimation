function [Eph, iono] = RINEX_get_nav(file_nav, constellations,OutputType)
...
%   OutputType = to obtain the correct parameters from an especific
%   constellation
% OUTPUT:
...
while (isempty(header_end))
    ...
    %if the ionosphere parameters label was found
    % Take ionosphere for GPS Glonass & Galileo
    if (~any(iono))
        % Ionospheric Galileo
        data = textscan(lin(5:end),'%f%f%f%f%*[^\n]');
%         if(version > 2)
%             if(strcmp(lin(1:3),'GAL') && (strcmp(OutputType,'E')))  % Ionospheric parameters Galileo
%                 if ~isempty(data{4})
%                     ionoGAL(1) = data{1};
%                     ionoGAL(2) = data{2};
%                     ionoGAL(3) = data{3};
...
%                 end
%                 iono = ionoGAL;
%             elseif(strcmp(lin(1:3),'GPS')) %Ionospheric parameters GPS & Glonass (Fabricio told me that)
%                 if ~isempty(data(4))
%                     iono(1) = data{1};
%                     iono(2) = data{2};
%                     iono(3) = data{3};
...
while (~feof(fid))
  ...
  %% Code from Carlos
    if(version > 2) %Version RINEX 3.XX
        year   = str2num(lin1([5:8]));
        month  = str2num(lin1([9:11]));
        day    = str2num(lin1([12:14]));
        hour   = str2num(lin1([15:17]));
        minute = str2num(lin1([18:20]));
        second = str2num(lin1([21:23]));
    elseif (sys_id == 'E') %If Galileo
        year   = str2num(lin1(o+[4:7])); 
        month  = str2num(lin1(o+[8:10]));
        day    = str2num(lin1(o+[11:13]));
        hour   = str2num(lin1(o+[14:16]));
        minute = str2num(lin1(o+[17:19]));
        second = str2num(lin1(o+[20:22]));    
    else % version < 3 && It is GPS or Glonass
    ...
    if (~strcmp(sys_id, 'R'))
        ...
        if(length(lin6) < 62) %Empty slot in Galileo
            L2flag = [];
        else
    ...
    %if GLONASS
        ...
        %% Carlos: Initially was commented
        ...
        Eph(19,i) = 0;
        Eph(20,i) = 0;
        Eph(21,i) = freq_L1;%frequencies on L1
        Eph(22,i) = freq_L2;%frequencies on L2
        Eph(23,i) = 0;
        Eph(24,i) = week_toe;
        Eph(25,i) = 0;
        Eph(26,i) = 0;
