function [fZZLB_DPE,fZZLB_CB] = computeZZBDPE_4d(config,sigen)
%% load data
eval(config)
...
%     alpha = sqrt(10^(SNRdb(SNRloop)/10)/NsamplesData);
    ...
    priorCov = blkdiag((Dmax0(SNRloop))^2*eye(3), (Dmax_clk0(SNRloop))^2);

%     ZZLB_DPE = priorCov(SNRloop)*2*q(sqrt(M*SNR))+(IFIM)^(-1)*gammainc(M*SNR/2,3/2);
%     ZZLB_DPE = Rd*2*q(sqrt(M*SNR))+(IFIM)^(-1)*gammainc(M*SNR/2,3/2);
    ZZLB_DPE = priorCov*2*q(sqrt(M*SNR))+(IFIM)^(-1)*gammainc(M*SNR/2,3/2);
