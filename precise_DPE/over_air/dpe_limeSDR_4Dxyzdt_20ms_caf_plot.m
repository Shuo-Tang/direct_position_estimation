...
for iSol = 28
    ...
    for idt = 1: ndt
        ...
        for iz = 1:nz
            for ix = 1: nx
                for iy = 1: ny                
                    % usrInfo = [xCan(ix), yCan(iy), 4207653.62, dtCan(idt),...%zCan(iz)
                    %     trueUsrInfo(5:end), settings.IF];
                    usrInfo = [trueUsrInfo, settings.IF];
                    % r(ix, iy) = generateCAFs_20ms_corr(usrInfo, [satInfo; PRN], rawSignal,...
                    %     caCodesTable, Doppler(:,(iSol-1)*1000 + 1), corr);
                    slope1(ix, iy) = generateCAFs_20ms_slope(usrInfo, [satInfo; PRN], rawSignal,...
                        caCodesTable, Doppler(:,(iSol-1)*1000 + 1), corr);
                end
            end
            rMax_z = max(r(:));
