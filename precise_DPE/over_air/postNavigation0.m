function [navSolutions, eph] = postNavigation0(trackResults, settings,acqResults)
...
sat_info = zeros(settings.numberOfChannels, 9, fix((settings.msToProcess - max(subFrameStart)) / ...
                                                     settings.navSolPeriod));
...
%% Find satellites positions and clocks corrections =======================
    [satPositions, satClkCorr, satVelocities] = satpos0(transmitTime,[trackResults(activeChnList).PRN],eph, settings);
...
%% Find receiver position =================================================
    ...
    if length(activeChnList) > 3
        ...
        % record for maltipath DPE
        sat_info(:, 1:3, currMeasNr) = satPositions';
        sat_info(:, 4:6, currMeasNr) = satVelocities';
        sat_info(:, 7, currMeasNr) = navSolutions.channel.az(activeChnList, currMeasNr);
        sat_info(:, 8, currMeasNr) = navSolutions.channel.el(activeChnList, currMeasNr);
        sat_info(:, 9, currMeasNr) = navSolutions.channel.PRN(activeChnList, currMeasNr);
...
% record for PDPE   
save("satInfo.mat", "satInfo");
% record for multipath
save('sat_info_multipath.mat', 'sat_info')