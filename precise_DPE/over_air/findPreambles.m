function [firstSubFrame, activeChnList, javi_subFrameStart_position] = findPreambles(trackResults, ...
                                                        settings)
        ...
        if (~isempty(find(index2 == 6000)))
            ...
            if (index(i)-40)>0
                ...
                if (navPartyChk(bits(1:32)) ~= 0) && ...
                    ...
                    if flag_fisrt_subframe_set==0
                        firstSubFrame(channelNr) = index(i);
                        flag_fisrt_subframe_set=1;
                    end
                    %javi: record all preamble positions
                    preamble_counter=preamble_counter+1;
                    javi_subFrameStart_position(channelNr,preamble_counter) =index(i);
                    %break;
                end % if parity is OK ...
