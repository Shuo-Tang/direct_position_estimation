function settings = initSettings_20ms()
...
settings.msToProcess        = 80000/20;%116000;%50000;        %[ms]
...
% based only. 
settings.skipNumberOfBytes     = (797 + 6) * 25375;% 63000;
...
settings.IF                 = -89.23;%420e3;%4.024e6 ;%[Hz]
...
settings.navSolPeriod       = 1000/20;          %[ms]
...
% estPos = [4777972.62497493, 176323.172246097, 4207664.68067328];
% estPos = [4777977.19425383, 176323.577906622, 4207669.59285189];
% X = estPos(1); Y = estPos(2); Z = estPos(3);
...
% 2000    10     3    17     0     0                        EPOCH OF CURRENT MAP
% 42.5-180.0 180.0   5.0 450.0                            LAT/LON1/LON2/DLON/H
% 141  142  148  160  178  190  197  201  205  214  226  238  249  259  270  281
% 294  310  327  343  356  366  373  377  379  381  385  392  402  413  423  430
% 427  415  400  390  382*  371  345  329  314  303  293  281  269  260  257  260
% 264  264  261  259  259  258  254  247  240  233  224  213  202  191  183  179
% 176  171  165  158  151  144  141  141  141
