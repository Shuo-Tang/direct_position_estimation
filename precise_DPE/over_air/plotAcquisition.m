...
oldAxis = axis(hAxes);
n_sats=length(acqResults.carrFreq);
axis  (hAxes, [0, n_sats, 0, oldAxis(4)]);
...
hold(hAxes, 'on');
bar (hAxes, acquiredSignals, 'FaceColor', [0.3 0.7 0.2]);
hold(hAxes, 'off');

% hold(hAxes, 'on');
% removeIndex = [13, 16, 29];
% removedSignals = zeros(1, 32);
% removedSignals(removeIndex) = acquiredSignals(removeIndex);
% bar (hAxes, removedSignals, 'FaceColor', [0.7 0.7 0.7]);
% hold(hAxes, 'off');

legend(hAxes, 'Not acquired signals', 'Acquired signals');
% legend(hAxes, 'Not acquired signals', 'Acquired signals', 'Removed Signals');