function [trackResults, channel]= tracking_1ms(fid, channel, settings)
...
%% set tracking raw signal recording
% rawSignal_dpe = cell(codePeriods/settings.navSolPeriod, 1);
...
%% Start processing channels ==============================================
    for channelNr = 1:settings.numberOfChannels
        ...
        if (channel(channelNr).PRN ~= 0)
            ...
            caCode_20ms = repmat(caCode, 1, 20);
            % Then make it possible to do early and late versions
            caCode_20ms = [caCode_20ms(end) caCode_20ms caCode_20ms(1)];
    ...
    %% Read next block of data ------------------------------------------------  
                ...
                %     fseek(fid, ...
                %     dataAdaptCoeff*(settings.skipNumberOfBytes + channel(channelNr).codePhase-1) +...
                %     (loopCnt - 1)*dataAdaptCoeff*blksize, ...
                %     'bof');
                % end
                ...
                codePhaseStep = codeFreq / settings.samplingFreq; % one sample represents how much code
                
                blksize = ceil((20*settings.codeLength-remCodePhase) / codePhaseStep);
               ...
               if (dataAdaptCoeff==1)
                ...
                %% record raw signal for DPE
                % if loopCnt >= 4854
                %     if (rem(loopCnt-4854, 1000) == 0)
                %         rawSignal_dpe{(loopCnt-4854)/1000 + 1} = rawSignal;
                %     end
                % end
