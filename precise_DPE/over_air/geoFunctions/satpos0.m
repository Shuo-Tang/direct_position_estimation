function [satPositions, satClkCorr, satVelocities] = satpos0(transmitTime, prnList, ...
                                             eph, settings) 
...
%% Find satellite's position ----------------------------------------------
    ...
    n0  = sqrt(GM / a^3); %
    %Mean motion
    n   = n0 + eph(prn).deltan; %mkdot

    %Mean anomaly
    M   = eph(prn).M_0 + n * tk; %mk
    ...
    nu   = atan2(sqrt(1 - eph(prn).e^2) * sin(E), cos(E)-eph(prn).e);%tak

    %Compute angle phi
    phi = nu + eph(prn).omega;%phik
    ...
    %Extra terms for velocity computation
    mkdot = n;
    ekdot = mkdot/(1 - eph(prn).e*cos(E));
    nudot = sin(E)*ekdot*(1 + eph(prn).e*cos(nu))/(sin(nu)*(1 - eph(prn).e*cos(E)));

    ukdot = nudot + 2*(eph(prn).C_us*cos(2*u) - eph(prn).C_uc*sin(2*u))*nudot;
    rkdot = a*eph(prn).e*sin(E)*n/(1 - eph(prn).e*cos(E)) + ...
        2*(eph(prn).C_rs*cos(2*u) - eph(prn).C_rc*sin(2*u))*nudot;
    ikdot = eph(prn).iDot + (eph(prn).C_is*cos(2*u) - eph(prn).C_ic*sin(2*u))*2*nudot;
    xpk = r*cos(u);
    ypk = r*sin(u);
    ...
    satVelocities(1, satNr) = (xpkdot - ypk*cos(i)*Omegadot)*cos(Omega) - ...
        (xpk*Omegadot + ypkdot*cos(i) - ypk*sin(i)*ikdot)*sin(Omega);
    satVelocities(2, satNr) = (xpkdot - ypk*cos(i)*Omegadot)*sin(Omega) + ...
        (xpk*Omegadot + ypkdot*cos(i) - ypk*sin(i)*ikdot)*cos(Omega);
    satVelocities(3, satNr) = ypkdot*sin(i) + ypk*cos(i)*ikdot;
