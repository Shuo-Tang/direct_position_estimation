...
fileID = fopen('C:\signals\test_ifen_III_1a\cap3_III_1a.bin');

sample_index1=1;
sample_index2=1;
sample_index3=1;

%     <FrontendMode name="III-1a (L1/L2/L5 standard)">
%         <Variant major="3" minor="1" micro="0"/>
%         <bitfile name="flexiband_III-1a.bit"/>
...
%             <vga3>L5</vga3>
%         </vgas>
%         <FrontendBand name="L2" bits="2" samples="2" complex="1" usb="1" vga="1" filterIdx="0" sampleRate="20"/>
%         <FrontendBand name="L1" bits="2" samples="2" complex="1" usb="1" vga="2" filterIdx="1" sampleRate="20"/>
%         <FrontendBand name="L5" bits="4" samples="2" complex="1" usb="1" vga="3" filterIdx="2" sampleRate="20"/>
%     </FrontendMode>

header_offset_bytes=6;
crc_bytes=4;
n_samples_per_frame=506; %(1024-6-4)/2 =507 BUT Hardware is transmitting 506!!! O_o
sample_mux_bytes=2;
        ...
        if (packet(1)==85 && packet(2)==170)
           ...
           if (enable_data_decoding==1)
                for n_sample=0:1:(n_samples_per_frame)
                    ...
                    %00000011
                    Q1=int8(bitand(sample_byte,3,'uint8'));
                    %00001100
                    I1=bitshift(sample_byte,-2,'uint8');
                    I1=int8(bitand(I1,3,'uint8'));
                    if I1>=2
                        I1=I1-4;
                    end
                    if Q1>=2
                    ...
                    %00110000
                    Q2=bitshift(sample_byte,-4,'uint8');
                    Q2=int8(bitand(Q2,3,'uint8'));
                    %11000000
                    I2=bitshift(sample_byte,-6,'uint8');
                    I2=int8(bitand(I2,3,'uint8'));
                    if I2>=2
                        I2=I2-4;
                    end
                    if Q2>=2
...
y1=read_complex_char_binary('C:\signals\test_ifen_III_1a\cap3_III_1a__L1.bin',1000);
...
y2=read_complex_char_binary('C:\signals\test_ifen_III_1a\cap3_III_1a__L2.bin',1000);
...
%  
   Fs3=40e6;
  y3=read_complex_char_binary('C:\signals\test_ifen_III_1a\cap3_III_1a__L5.bin',1000);
