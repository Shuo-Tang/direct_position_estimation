...
classdef goGNSS < handle
    ...
    methods (Static, Access = 'public')
        ...
        function [constellations] = initConstellation(GPS_flag, GLO_flag, GAL_flag, BDS_flag, QZS_flag, SBS_flag)
            ...
            GPS_PRN = [1:32];
            %% Carlos:There are 26 sat in GLONASS, 
            GLO_PRN = [1:26];
%            GLO_PRN = [1:24];
            %% Carlos:There are just 30 sat BUT their index can go from 1 to 36, 
            %so we suppose that there are up to 36, the one that won't appear will count as it is not used.
            GAL_PRN = [1:36];   
            %%
%             GAL_PRN = [1:30];
            BDS_PRN = [1:37];
        ...
        function [pr1_R, pr1_M, ph1_R, ph1_M, pr2_R, pr2_M, ph2_R, ph2_M, ...
            ...
            if (~flag_SP3)
                ...
                if (constellations.Galileo.enabled)
                    if (exist([filename_nav(1:end-1) 'l'],'file')) %Carlos
