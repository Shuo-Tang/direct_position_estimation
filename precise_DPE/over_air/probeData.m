...
if (fid > 0)
    ...
    % records).
    fseek(fid, max(settings.skipNumberOfBytes), 'bof');

    % Find number of samples per spreading code (sampling number on every 1023)
   ...
   if (settings.fileType==4)
    [data, count] = fread(fid, [2, 100*samplesPerCode], settings.dataType);
   ...
   if (settings.fileType==8)
    [data, count] = fread(fid, [2, 100*samplesPerCode], settings.dataType);
    data = data(1,:) + data(2,:).*i; %Inphase and Quadrature
   end 
   
    fclose(fid); 
    if (settings.fileType==4)
        if (count < 2*100*samplesPerCode)
