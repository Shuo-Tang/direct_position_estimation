function [x_tx, pseudorange] = signalGenerate_iono(config, usrInfo, satInfo, caCode, iono, tropo)
...
pseudorange = zeros(nSat, 1);
%% generate local replica
for iSat = 1:nSat
    Code = caCode(satPRN(iSat),:);
    %% delay
    GeoRange = norm(satPos(iSat,:) - usrPos);
    Range = GeoRange + c*(usrCb - satCb(iSat)) + iono(iSat) + tropo(iSat);
