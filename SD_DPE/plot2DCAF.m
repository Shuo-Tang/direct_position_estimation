...
% alpha(h, 0.5);
hold on
xlabel("Y - Y_{true}", "FontSize",14); ylabel("X - X_{true}", "FontSize",14); zlabel("Cost")
% xlabel("Y - Y_{true} (ECEF)"); ylabel("X - X_{true} (ECEF)"); zlabel("Cost")
% plot max
rMax = max(r(:));
[yMax,xMax] = find(r == rMax);
h_max = plot3(xCan(xMax)-truePos(1), yCan(yMax)-truePos(2), r(yMax, xMax),...
    'r+', 'LineWidth', 2.5, 'MarkerSize', 10);
xlim([-80, 80]);
ylim([-80, 80])
legend(h_max, 'Optimum Solution', FontSize=14)
% set view
view  (0, 90);
% axis  ('equal');
grid  ('minor'); 
