...
nIono = length(iono_candidates);
pos_error_iono_DPE = zeros(nIono, 1);
pos_error_iono_SD = zeros(nIono, 1);
for iIono = 50%1:nIono
    iono_error = iono_candidates(iIono);
    ...
    pos_error_iono_SD(iIono) = sqrt(sum(error_norm_SD.^2)/200);
    error_DPE = pos_sol_DPE - rov_info(1:3);
    error_norm_DPE = vecnorm(error_DPE, 2, 2);
    pos_error_iono_DPE(iIono) = sqrt(sum(error_norm_DPE.^2)/200);
end

% save("result/monte_carlo/pos_error_iono_SD", "pos_error_iono_SD")
% save("result/monte_carlo/pos_error_iono_DPE", "pos_error_iono_DPE")