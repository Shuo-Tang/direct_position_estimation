...
nCNo = length(cno_candidates);
pos_error_CNo_DPE = zeros(nCNo, 1);
pos_error_CNo_SD = zeros(nCNo, 1);
for iCNo = 1:nCNo
    CNo = cno_candidates(iCNo);
    ...
    pos_error_CNo_SD(iCNo) = sqrt(sum(error_norm_SD.^2)/200);
    error_DPE = pos_sol_DPE - rov_info(1:3);
    error_norm_DPE = vecnorm(error_DPE, 2, 2);
    pos_error_CNo_DPE(iCNo) = sqrt(sum(error_norm_DPE.^2)/200);
end

% save("result/monte_carlo/pos_error_CNo_SD", "pos_error_CNo_SD")
% save("result/monte_carlo/pos_error_CNo_DPE", "pos_error_CNo_DPE")